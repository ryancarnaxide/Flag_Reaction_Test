...
export default function App() {
  ...
  // Magnet status
  const [magnetsOn, setMagnetsOn] = useState(false);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      // trigger hardware drop sequence
      ...
      // Go to ready screen after flash
      setView("ready");
    }, 1000);

    return () => clearTimeout(timer);
  }, [view, hardwareAvailable, difficulty]);

  // READY: Slideshow plays 8 seconds → CAPTURE
  useEffect(() => {
    if (view !== "ready") return;

    const timer = setTimeout(() => {
      setView("capture");
      setMagnetsOn(false); // toggle will turn red on home
    }, 8000); 

    return () => clearTimeout(timer);
  }, [view]);
  
  // creating helper func for the magnet switch toggle
  async function handleMagnetToggle(isOn) {
    setMagnetsOn(isOn);

    try {
      if (isOn) {
        await turnMagnetsOn();   // 🔥 API call
        showMessage("Magnets ON");
      } else {
        // setting up an alert message to tell player to remove flags
        window.alert("The magnets will turn off. Please remove all batons then hit OK.");
        await turnMagnetsOff();  // 🧊 API call
        showMessage("Magnets OFF");
      }
    } catch (error) {
      showMessage("Hardware error: " + error.message);
    }
  ...
  return (
    <div className="page">
      ...
      <BackgroundSlideshow />
      {/* TOP BAR: Logo always visible, toggle only on home screen */}
      <div className="top-bar">
        ...
        {/* Magnet Status + optional toggle */}
        <div className="top-right">
          {/* Status text for magnets */}
          <p className="magnet-status">
            Magnets are currently{" "}
            <span className={magnetsOn ? "on-text" : "off-text"}>
              {magnetsOn ? "ON" : "OFF"}
            </span>
          </p>

          {/* Only show toggle on home screen */}
          {view === "home" && (
            <label className={`magnet-toggle ${magnetsOn ? "on" : "off"}`}>
              <input
                type="checkbox"
                checked={magnetsOn}
                onChange={(e) => handleMagnetToggle(e.target.checked)}
              />
                      ...
                      return (
                        <li
                          ...
                          onClick={async () => {
                            ...
                            setView("difficulty");
                            // try {
                            //   // 1️⃣ Check if all magnets are on
                            //   const res = await fetch("http://127.0.0.1:8000/hardware/magnets/status");
                            //   const data = await res.json();

                            //   if (!data.all_on) {
                            //     // 2️⃣ Turn magnets on if any are off
                            //     try {
                            //       await turnMagnetsOn();
                            //       showMessage("✓ All magnets ON");
                            //     } catch (error) {
                            ...
                            //   // 3️⃣ Proceed to difficulty screen AFTER magnets ready
                            //   setView("difficulty");
                            // } catch (error) {
                            //   showMessage("Error checking/turning magnets: " + error.message);
                            // }
                          }}
                          role="option"
      ...
      {/* READY - PLAYS SLIDESHOW UNTIL DROP SEQUENCE FINISHES */}
      {view === "ready" && (
        <Screen view={view}>
          <div className="home">
          </div>
              ...
              <div className="nums-vertical">
                {Array.from({ length: 11 }, (_, i) => i).map((num) => (
                  <button
                    ...
                    onClick={() => {
                      setCatches(num); // set difficulty
                      handleSaveSession(); // immediately start countdown
                    }}
                  >
                    {num}
                  </button>
      ...
      {view === "leaderboard" && (
        <Screen view={view}>
          <div className="panel">
            ...
            {/* Adding a magnets on call to the test again button */}
            <div className="actions">
              <button
                className="btn small"
                onClick={() => setView("select")}
